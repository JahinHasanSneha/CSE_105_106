...
void surprise_insert(int item, linkedList *list, int count)
{
    if (count == 1)
        insert(item, &list);
    else
...
void combine(linkedList *list)
{
    ...
    node *cur = list->head;
    for (int i = 0; i < list->current && cur != NULL; i++)
    {
        cur = cur->next;
...
void reverse_range(int k, linkedList *list)
{
    ...
    while (start != NULL)
    {
        ...
        // Move end to kth node or last node
        ...
        // Reverse values between start and end
        node *left = start;
        node *right = end;
...
void rotate(int r, linkedList *list)
{
    if (list->length <= 1)
        return;
    if (r == 0)
...
void skip_delete(int k, linkedList *list)
{
